  ...
  class TimeSeries < ApplicationRecord
    ...
    def append_to_tsv(append_text, headers, filename)
      ...
      if !self.file.file
        ...
        logger.info "Creating initial time series with #{rows} rows to #{self.file.path}"
      else
        unless File.exists? self.file.path
          logger.warn "Time Series file doesn't exist: #{self.file.path}"
          dir = File.dirname(self.file.path)
          FileUtils.mkdir_p dir
          File.open(self.file.path, 'w') {|file| file.write(headers.map(&:to_s).join("\t") + "\n")}
        end
    ...
    def append_file_to_tsv(append_file, headers, filename)
      unless self.file.file
        self.file = FileIO.new(headers.map(&:to_s).join("\t") + "\n", filename)
        logger.info "Creating initial time series with header to #{self.file.path}"
        save!
      end
